...
export default function DashboardLayout() {
  ...
  return (
    <div className="flex h-screen bg-slate-950 text-slate-100 overflow-hidden font-sans dark select-none">
      {/* Desktop Sidebar */}
      <aside className="w-72 bg-slate-950 text-slate-400 flex flex-col hidden lg:flex h-full border-r border-slate-800/80 relative z-30 shadow-2xl shrink-0" aria-label="Navegação Lateral">
        <div className="p-6 border-b border-slate-800/80 flex flex-col gap-5">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl overflow-hidden shadow-xl border border-amber-400/30 shrink-0 bg-slate-900 flex items-center justify-center font-black italic text-xs text-white">
              {!logoError ? (
                <img 
                  src={logoSrc} 
                  alt="RA Logo" 
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] text-amber-400 font-extrabold uppercase tracking-widest mt-1 flex items-center gap-1">
          ...
          <Link to="/" aria-label="Voltar para o site público">
            <Button variant="secondary" className="w-full bg-slate-900 hover:bg-slate-800 text-slate-300 hover:text-white justify-start gap-2 h-10 rounded-xl border border-slate-800 transition-all text-xs font-bold">
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3.5 px-3.5 py-3 rounded-xl transition-all duration-200 group font-bold text-sm ${
                    isActive 
                      ? 'bg-amber-400 text-slate-950 font-black shadow-lg shadow-amber-500/10 scale-[1.01]' 
                      : 'text-slate-400 hover:bg-slate-900 hover:text-slate-100'
                  }`}
                >
                  <Icon className={`w-5 h-5 transition-transform ${isActive ? 'text-slate-950' : 'text-slate-400 group-hover:text-amber-400'}`} aria-hidden="true" />
        ...
        <div className="p-4 border-t border-slate-800/80 bg-slate-900/60 backdrop-blur-sm flex flex-col gap-3">
          <div className="flex items-center gap-3 p-2.5 bg-slate-900/90 rounded-xl border border-slate-800">
            <div className="w-9 h-9 rounded-lg bg-amber-400/20 border border-amber-400/30 flex items-center justify-center text-amber-400 font-black text-sm">
      ...
      <div className="flex-1 flex flex-col h-full relative overflow-hidden bg-slate-950">
        ...
        {/* Top Header Bar */}
        <header className="bg-slate-950/90 backdrop-blur-xl border-b border-slate-800/80 px-4 py-3 flex items-center justify-between sticky top-0 z-40">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl overflow-hidden shadow-md border border-amber-400/40 shrink-0 bg-slate-900 flex items-center justify-center font-black italic text-[10px] text-white">
              {!logoError ? (
                <img 
                  src={logoSrc} 
                  alt="RA Logo" 
            ...
            <div>
              ...
              <span className="text-[10px] font-extrabold text-amber-400 uppercase tracking-wider flex items-center gap-1 mt-0.5">
                <span className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse"></span>
                Painel Modo Obra
              </span>
            </div>
          ...
          <div className="flex items-center gap-2">
            <Link to="/" className="hidden sm:inline-flex">
              <Button size="sm" variant="outline" className="border-slate-800 bg-slate-900/80 hover:bg-slate-800 text-slate-300 hover:text-white font-bold rounded-xl text-xs h-9 px-3">
            ...
            <Button 
              variant="ghost" 
              size="icon" 
              className="lg:hidden text-slate-300 hover:text-white hover:bg-slate-900 rounded-xl h-9 w-9 border border-slate-800" 
        ...
        {isMeuSiteTab && (
          <div className="bg-slate-900/90 backdrop-blur-md border-b border-slate-800/80 px-4 py-2.5 flex items-center justify-center gap-2 sticky top-[57px] z-30 shadow-md">
            <Link
              to="/app/portfolio"
            ...
            >
              ...
              {newLeadsCount > 0 && (
                <span className={`ml-1 text-[9px] font-black h-4 px-1 rounded-full flex items-center justify-center animate-pulse ${
                  location.pathname.startsWith('/app/leads') ? 'bg-slate-950 text-amber-400' : 'bg-amber-400 text-slate-950'
                }`}>
                  {newLeadsCount}
                </span>
              )}
        ...
        {isOffline && (
          <div className="bg-amber-500 text-slate-950 font-black text-xs uppercase tracking-widest py-2 px-4 text-center flex items-center justify-center gap-2 sticky top-[57px] z-30 shadow-md" role="alert">
            ...
            {isRootApp && (
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                  {/* [📝 NOVA O.S. / Gerar Relatório] Em destaque Amarelo */}
                  <Link 
                    to="/app/orders" 
                    className="group bg-amber-400 hover:bg-amber-300 text-slate-950 p-5 md:p-6 rounded-2xl shadow-xl shadow-amber-500/10 border border-amber-300 transition-all transform active:scale-[0.98] flex items-center justify-between"
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="text-left">
                        <div className="text-[10px] font-black uppercase tracking-widest text-slate-800">EM CAMPO</div>
                        <div className="text-base md:text-lg font-black leading-tight tracking-tight uppercase">
                          📝 NOVA O.S.
                        </div>
                        <div className="text-xs font-bold text-slate-800 opacity-90">
                  ...
                  {/* [💰 ORÇAMENTO / Proposta Comercial] Em Destaque Escuro/Borda */}
                  <Link 
                    to="/app/quotes" 
                    className="group bg-slate-900 hover:bg-slate-850 text-white p-5 md:p-6 rounded-2xl shadow-xl border-2 border-amber-400/50 hover:border-amber-400 transition-all transform active:scale-[0.98] flex items-center justify-between"
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-xl bg-amber-400/10 border border-amber-400/30 text-amber-400 flex items-center justify-center shrink-0 shadow-md">
                        <FileText className="w-6 h-6" />
                      </div>
                      <div className="text-left">
                        <div className="text-[10px] font-black uppercase tracking-widest text-amber-400">COMERCIAL</div>
                        <div className="text-base md:text-lg font-black leading-tight tracking-tight uppercase">
                          💰 ORÇAMENTO
                        </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <Link
                    to="/app/clients"
                    className="bg-slate-900/90 hover:bg-slate-850 text-slate-200 border border-slate-800 hover:border-amber-400/40 p-3.5 rounded-xl flex items-center justify-between font-extrabold text-xs sm:text-sm transition-all group"
                  >
                    <div className="flex items-center gap-3">
        ...
        <nav 
          className="fixed bottom-0 left-0 right-0 bg-slate-950/95 backdrop-blur-xl border-t border-slate-800/90 z-50 px-3 py-1.5 shadow-2xl flex justify-around items-center"
        ...
        >
          ...
          <Link
            ...
            className={`flex flex-col items-center py-1.5 px-4 rounded-xl transition-all relative ${
              isEmCampoTab ? 'text-amber-400' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            {isEmCampoTab && (
              <motion.div 
            ...
            <HardHat className={`w-5 h-5 relative z-10 transition-transform ${isEmCampoTab ? 'scale-110 text-amber-400' : ''}`} />
          ...
          <Link
            to="/app/portfolio"
            className={`flex flex-col items-center py-1.5 px-4 rounded-xl transition-all relative ${
              isMeuSiteTab ? 'text-amber-400' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            {isMeuSiteTab && (
              <motion.div 
            ...
            <div className="relative">
              <Globe className={`w-5 h-5 relative z-10 transition-transform ${isMeuSiteTab ? 'scale-110 text-amber-400' : ''}`} />
              {newLeadsCount > 0 && (
                <span className="absolute -top-1 -right-2 bg-amber-400 text-slate-950 text-[9px] font-black h-4 w-4 rounded-full flex items-center justify-center animate-pulse z-20">
                  {newLeadsCount}
                </span>
              )}
            </div>
          ...
          <Link
            to="/app/settings"
            className={`flex flex-col items-center py-1.5 px-4 rounded-xl transition-all relative ${
              isSettingsTab ? 'text-amber-400' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            {isSettingsTab && (
              <motion.div 
            ...
            <Sliders className={`w-5 h-5 relative z-10 transition-transform ${isSettingsTab ? 'scale-110 text-amber-400' : ''}`} />
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div 
              ...
              className="fixed inset-0 bg-slate-950 text-slate-100 z-50 lg:hidden flex flex-col"
            >
              <div className="p-5 border-b border-slate-800 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-[10px] text-amber-400 font-extrabold uppercase">Menu de Navegação</span>
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-2">
                <Link to="/" onClick={() => setIsMobileMenuOpen(false)} className="block mb-4">
                  <Button variant="secondary" className="w-full bg-slate-900 text-slate-300 hover:bg-slate-800 justify-start gap-2 h-11 rounded-xl border border-slate-800 font-bold text-xs">
                     <ArrowLeft className="w-4 h-4 text-amber-400" />
                ...
                {navItems.map(item => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="flex-1">{item.name}</span>
                      {item.path === '/app/leads' && newLeadsCount > 0 && (
                        <span className="text-[10px] font-black h-5 px-1.5 rounded-full bg-amber-400 text-slate-950">
                          {newLeadsCount}
                        </span>
                      )}
                    </Link>
        ...
        <AnimatePresence>
          {activeNotification && (
            <motion.div
              ...
              className="fixed bottom-20 lg:bottom-10 right-4 z-50 max-w-sm w-[calc(100%-2rem)] bg-slate-900 text-white border border-amber-400/40 p-4 rounded-2xl shadow-2xl backdrop-blur-xl flex flex-col gap-3 font-sans"
              role="alert"
            >
              ...
              <div className="flex gap-3.5 items-start pr-6">
                <div className="bg-amber-400/20 p-2.5 rounded-xl text-amber-400 relative shrink-0">
                  <div className="absolute inset-0 bg-amber-400/30 rounded-xl animate-ping opacity-40"></div>
              ...
              <div className="flex gap-2 mt-1">
                <Link 
                  ...
                  className="flex-1"
                >
                  <Button className="w-full h-9 bg-amber-400 hover:bg-amber-300 text-slate-950 font-black text-xs rounded-xl shadow-md uppercase tracking-wider flex items-center justify-center gap-1.5">

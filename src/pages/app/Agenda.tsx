      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
         ...
         <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-[#1E293B] rounded-[2rem] p-6 border border-slate-800 shadow-xl space-y-4">
               <div>
                  <h4 className="font-black text-sm text-white uppercase tracking-widest italic">Análise Agenda</h4>
               ...
               <div className="space-y-3 pt-2">
                  <div className="flex justify-between items-center text-xs border-b border-slate-800 pb-2.5">
                     <span className="text-slate-400 font-bold uppercase tracking-wider text-[10px]">Total Geral</span>
                     <span className="font-black text-[#EAB308] bg-[#0B0F19] border border-slate-800 px-2.5 py-0.5 rounded-full">{events.length}</span>
                  </div>
                  <div className="flex justify-between items-center text-xs border-b border-slate-800 pb-2.5">
         ...
         <div className="lg:col-span-3 space-y-4">
              ...
              {events.map((evt, i) => {
                 ...
                 return (
                   <motion.div 
                     ...
                     className={`group bg-[#1E293B] rounded-3xl shadow-xl border border-slate-800 overflow-hidden flex items-center justify-between p-5 md:p-6 hover:border-[#EAB308]/40 transition-all ${isPast ? 'opacity-65 grayscale-[0.3]' : ''}`}
                   >
                      <div className="flex items-center gap-4 md:gap-6">
                         <div className={`p-4 rounded-2xl flex flex-col items-center justify-center min-w-[76px] transition-colors ${isPast ? 'bg-[#0B0F19] text-slate-500 border border-slate-800' : 'bg-[#0B0F19] text-[#EAB308] border border-slate-800'}`}>
                         ...
                         <div>
                            <div className="flex flex-wrap items-center gap-2 mb-1.5">
                               <h3 className={`font-black text-lg md:text-xl tracking-tight leading-tight ${isPast ? 'line-through text-slate-500' : 'text-white'}`}>{evt.title}</h3>
                               <span className={`text-[9px] px-2.5 py-0.5 rounded-full font-black uppercase tracking-widest border ${
                                 evt.type === 'service' ? 'bg-amber-950/80 text-amber-400 border-amber-800' : 
                                 evt.type === 'visit' ? 'bg-emerald-950/80 text-emerald-400 border-emerald-800' : 
                            ...
                            <div className="flex flex-wrap items-center text-xs font-bold text-slate-400 gap-y-1.5 gap-x-5">
                               <span className="flex items-center gap-1"><Clock className="w-4 h-4 text-slate-500"/> {format(evt.date, 'HH:mm')}</span>
                               {evt.description && <span className="flex items-center gap-1.5 max-w-[200px] md:max-w-md truncate text-slate-400 italic">
                                  <CheckCircle2 className="w-3.5 h-3.5 text-slate-500" /> {evt.description}
                               </span>}
                            </div>
                      ...
                      <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                         <Button variant="ghost" size="icon" className="text-slate-400 hover:text-rose-400 hover:bg-rose-950/40 rounded-full" onClick={() => setEventToDelete({ id: evt.id, title: evt.title })}>
            ...
            {events.length === 0 && (
              ...
              >
                ...
                <h3 className="text-white font-extrabold text-xl uppercase tracking-tight italic">Sua agenda está vazia</h3>
